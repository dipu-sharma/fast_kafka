...
async def deposit(db: AsyncSession, transaction: TransactionCreate):
    async with db.begin():
        # Get account
        result = await db.execute(select(Account).where(Account.id == transaction.account_id).with_for_update())
...
async def transfer(db: AsyncSession, transfer: TransferCreate):
    async with db.begin():
        # Get from_account
        result_from = await db.execute(select(Account).where(Account.id == transfer.from_account_id).with_for_update())
        from_account = result_from.scalar_one_or_none()
        
        # Get to_account
        result_to = await db.execute(select(Account).where(Account.id == transfer.to_account_id).with_for_update())
